...
import { companyPopularSearches } from "@/data/companies";
...
export function CompaniesHero() {
  return (
    <section className="relative overflow-hidden bg-ghost">
      ...
      <div className="landing-container relative pb-16 pt-8 lg:pb-20 lg:pt-16">
        <div className="relative z-10 max-w-[850px]">
          <div className="relative mb-6 max-w-[640px] lg:mb-8">
          ...
          <p className="mt-5 text-base leading-6 text-muted">
            ...
            {companyPopularSearches.map((term, index) => (
              <span key={term}>
                <a
                  href="#"
                  className="underline decoration-muted/60 underline-offset-4 hover:text-primary"
                >
                  {term}
                </a>
                {index < companyPopularSearches.length - 1 ? ", " : ""}
              </span>
            ))}
          </p>
        </div>
